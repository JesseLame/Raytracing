...
namespace Template
{
	public class OpenTKApp : GameWindow
	{
		...
		protected override void OnKeyPress(OpenTK.KeyPressEventArgs e)
		{
			...
			//Distance to plane greater
			else if (e.KeyChar == 'x')
			{
				raytracer.camera.distToCamera += 0.5f;
				Console.WriteLine("X");
			}
			//Distance to plane smaller
			//Never smaller than 1
			else if (e.KeyChar == 'z')
            {
				float newDis = raytracer.camera.distToCamera -= 0.5f;
				if (newDis >= 1f)
					raytracer.camera.distToCamera = newDis;
			...
			{
				raytracer.camera.viewDegree += 5f;
				Console.WriteLine("N");
			}
			//Reduce FOV
			//Never smaller than 30
			else if (e.KeyChar == 'z')
			{
				float newDis = raytracer.camera.distToCamera -= 5f;
				if (newDis >= 30f)
					raytracer.camera.distToCamera = newDis;
